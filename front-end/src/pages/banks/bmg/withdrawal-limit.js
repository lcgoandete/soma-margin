...
import { useWithdralwalLimit } from '../../../hooks/banks/bmg/useWithdralwalLimit';
...
export const WithdrawalLimit = () => {
  ...
  const { getWithdralwalLimit, loading } = useWithdralwalLimit();
  ...
  const getWithdralwalLimits = async (event) => {
    ...
    const result = await getWithdralwalLimit(payload);
