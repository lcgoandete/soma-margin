...
const checkCpf = async (driver, cpf) => {
  ...
  if (invalidCpf.length > 0) {
    ...
    const newError = { status: NotFound, message: error };
    throw newError;
  }
};
...
const getIdentificationDada = async (driver, marginSelector) => {
  const dadosIdentificacao = {
    cpf: '', nome: '', orgao: '', identificacao: '', mesReferencia: '', dataProcessamento: '',
  };

  for (const key in dadosIdentificacao) {
    const element = await driver
      .findElements(By.xpath(selector[marginSelector].dadosIdentificacao[key]));
    if (element.length > 0) {
      dadosIdentificacao[key] = await driver
        .findElement(By.xpath(selector[marginSelector].dadosIdentificacao[key])).getText();
    }
  }
...
const getRawMargin = async (driver, marginSelector) => {
  const margemBruta = {
    provimento: '',
    consignacoesFacultativas: '',
    cartaoCredito: '',
    cartaoBenefico: '',
    dadosFuncionais: {
      lotacao: '', cargo: '', dataAdmissao: '', tipoVinculo: '', dataFinalContrato: '',
    },
  };

  const margemBrutaCredito = await driver
    .findElements(By.xpath(selector[marginSelector].margemBruta.cartaoCredito));

  if (margemBrutaCredito.length > 0) {
    for (const key in margemBruta) {
      if (key !== 'dadosFuncionais') {
        const element = await driver
          .findElements(By.xpath(selector[marginSelector].margemBruta[key]));
        if (element.length > 0) {
          margemBruta[key] = await driver
            .findElement(By.xpath(selector[marginSelector].margemBruta[key])).getText();
        }
      }
    }

    const dadosFuncionaisBotao = await driver
      .findElements(By.xpath(selector[marginSelector].margemBruta.dadosFuncionais.botao));
    if (dadosFuncionaisBotao.length > 0) {
      await driver
        .findElement(By.xpath(selector[marginSelector].margemBruta.dadosFuncionais.botao)).click();
      await driver.sleep(1000);
    }

    for (const key in margemBruta.dadosFuncionais) {
      const element = await driver
        .findElements(By.xpath(selector[marginSelector].margemBruta.dadosFuncionais[key]));
      if (element.length > 0) {
        margemBruta.dadosFuncionais[key] = await driver
          .findElement(By.xpath(selector[marginSelector].margemBruta.dadosFuncionais[key])).getProperty('value');
      }
    }
  }
  return margemBruta;
...
const availableMargin = async (driver, marginSelector) => {
  const margemDisponivel = {
    provimento: '', consignacoesFacultativas: '', cartaoCredito: '', cartaoBenefico: '',
  };

  for (const key in margemDisponivel) {
    const element = await driver
      .findElements(By.xpath(selector[marginSelector].margemDisponivel[key]));
    if (element.length > 0) {
      margemDisponivel[key] = await driver
        .findElement(By.xpath(selector[marginSelector].margemDisponivel[key])).getText();
    }
  }
