...
//     '#tie_form > div > div:nth-child(2) > input[type=text]',
//   )).sendKeys(USER_RUMOLA);

//   await driver.findElement(By.css(
//     '#tie_form > div > div:nth-child(3) > input[type=password]',
//   )).sendKeys(PASSWORD_RUMOLA);

//   await driver.findElement(By.css(
//     '#tie_form > div > div:nth-child(4) > input[type=submit]',
//   )).click();
...
const loginPortalDoConsignado = async (driver) => {
  const { USER_PORTAL_API } = process.env;
  const { PASSWORD_PORTAL_API } = process.env;
  ...
  await driver.findElement(By.css('#username')).sendKeys(USER_PORTAL_API);
  await driver.findElement(By.css('#password')).sendKeys(PASSWORD_PORTAL_API);
  await driver.sleep(1000);
